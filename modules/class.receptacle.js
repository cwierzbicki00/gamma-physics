...
// Contains a base class for a receptacle object, which is
// intended to be a target for the player to toss the throwable at.
...
class Receptacle {
  ...
  // return true if the ball scores
  ...
  OnCollisionEnter(ball) {
    for (let i = 0; i < this.edges.length; i++) {
      ...
      let ballImgOnEdge = lineLineIntersection(
        edgeStart,
        edgeEnd,
        ball.pos,
        ballPerpenEnd
      ...
      if (isPointInBetween(edgeStart, edgeEnd, ballImgOnEdge) == 1) {
        if (
          dist(ball.pos.x, ball.pos.y, ballImgOnEdge.x, ballImgOnEdge.y) <
          ball.getRadius()
        ) {
          if (i == 0) {
            return true;
          } else {
            let newP = ballPosAfterCollide(
              ball.pos,
              ballImgOnEdge,
              ball.getRadius()
            );
